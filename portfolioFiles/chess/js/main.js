...
function getAllPossibleCoordsKing(pieceCoord) {
    var res = [];
    for (var idx = pieceCoord.i-1; idx <= pieceCoord.i+1; idx++) {
        if (idx < 0 || idx >= gBoard.length) continue;
        for (var jdx = pieceCoord.j-1; jdx <=pieceCoord.j+1 ; jdx++) {
            if (jdx < 0 || jdx >= gBoard[idx].length) continue;
            if (idx === pieceCoord.i && jdx === pieceCoord.j) continue;
